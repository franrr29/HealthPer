...
**Live app:** [healthper.online](https://healthper.online) · **Try it without an account:** click **"Recruiter Preview"** on the login screen. It logs you in as a demo doctor with sample patients and consultations already loaded — no credentials needed. It's gated by an `ALLOW_DEMO` env flag on the backend, so it can be disabled without touching code.

## 🔄 Consultation Flow

The core workflow follows a multi-step pipeline from live audio to the patient's inbox:

1. **🎙️ Record** — The doctor starts a live recording. The system captures multi-speaker clinical audio in the browser.
2. **⏸️ Pause & Get Suggested Questions** — The doctor can pause at any point. The system analyzes the current conversation combined with the patient's full medical history and generates context-aware follow-up questions the doctor might want to ask.
3. **▶️ Resume** — The doctor resumes recording and continues the consultation. This pause/resume cycle can happen as many times as needed.
4. **⏹️ Stop & Transcribe** — When the consultation is complete, the doctor stops the recording. The audio is transcribed with Whisper (via Groq) preserving clinical terminology and speaker turns.
5. **📋 Summarize** — A structured clinical summary (chief complaint, symptoms, diagnosis, treatment, follow-up) is generated with Llama 3.3 70B. The doctor can review and edit it before signing.
6. **📧 Deliver** — The signed summary is sent to the patient's inbox as a clear, plain-language email via Resend — without blocking the clinical workflow.

## ⚙️ What happens behind the scenes

- Signing a consultation locks it with a server-side timestamp and triggers two background jobs: the transcript is chunked, embedded (Gemini) and indexed for retrieval, and the patient's long-term memory (chronic conditions, allergies, medications, recurring symptoms, master summary) is merged with the new summary through another LLM call.
...
- Auth supports email/password, Google OAuth, and the one-click demo login described above.
- The dashboard shows pending signatures, recent activity, and the doctor's most common chronic conditions/allergies across all patients.

## 🧱 Stack

| Layer | Tech |
|---|---|
| **Frontend** | React 19, TypeScript, Vite, TanStack Query, React Router v7, Tailwind CSS v4, Radix UI / shadcn, Motion (Framer Motion), Axios |
| **Backend** | Node.js, Express 5, TypeScript, Zod, JWT + httpOnly cookies, Passport (Google OAuth 2.0) |
| **AI / LLMs** | Groq (Llama 3.3 70B for summaries/chat/suggested questions, Whisper large-v3 for transcription), Google Gemini (`gemini-embedding-001` for embeddings) |
| **Database** | MySQL (`mysql2`) — hybrid retrieval via in-app cosine similarity + native MySQL FULLTEXT, no dedicated vector DB |
| **Email** | Resend (transactional email with patient-friendly summaries) |
| **Monitoring** | UptimeRobot (keep-alive ping for Render free tier) |
| **Deployment** | Render (backend), Vercel (frontend) |
| **Testing** | Jest, Supertest, ts-jest |

## 🏗️ Architecture

The backend is organized into domain modules (`auth`, `patient`, `doctor`, `consultation`, `email`, `ai`) instead of flat layer folders — each module keeps its controller, service and routes together. `ai/` is the exception: it has no routes of its own, it's internal infrastructure consumed by `consultation` (summaries, indexing, suggested questions) and `patient` (RAG chat).
...
**Suggested questions:** when the doctor pauses the recording, the current partial transcript is sent along with the patient's stored medical history to the LLM. The model generates targeted follow-up questions that help the doctor cover diagnostic gaps — grounded in what was already discussed and what the patient's history suggests.
...
**Context-aware suggested questions over generic templates.** Instead of showing a static list of follow-up questions, the system builds the prompt from the live transcript plus the patient's full history. This means the suggestions adapt to what has already been discussed and what the patient's background indicates — a patient with a history of hypertension gets different follow-ups than one with asthma.
