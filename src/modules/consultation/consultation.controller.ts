...
//funcion para traer consultas pendientes de firma del doctor logeado y mostrar en dashboard:
export async function getPendingConsultationsController(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    const { id: doctor_id } = req.user;
    const pendingConsultations = await getPendingConsultationsService(doctor_id);
